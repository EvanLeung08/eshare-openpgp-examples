...
public class PGP
{
    ...
    private static Iterator<PGPPublicKeyEncryptedData> getEncryptedObjects(final byte[] data) throws IOException {
        final PGPObjectFactory factory = new PGPObjectFactory(PGPUtil.getDecoderStream(new ByteArrayInputStream(data)),new JcaKeyFingerprintCalculator());
    ...
    private static PGPLiteralData asLiteral(
        ...
        final PGPSecretKeyRingCollection keys = new PGPSecretKeyRingCollection(new ArmoredInputStream(keyfile),new JcaKeyFingerprintCalculator());
